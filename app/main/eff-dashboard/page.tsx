    ...
    return (
        <>
            <h2 className="text-xl font-bold text-blue-600 mb-4 flex items-center">
                Production Efficiency by Phase
            </h2>
                        ...
                        {metrics.map((metric) => (
                            <tr key={metric.phase} className="hover:bg-blue-50/50">
                                ...
                                <td className={`px-6 py-4 whitespace-nowrap text-2xl font-extrabold text-right align-middle ${
                                    parseFloat(metric.efficiency) >= 95 ? 'text-green-600' : 
                                    parseFloat(metric.efficiency) >= 80 ? 'text-yellow-600' : 'text-red-600'
                                }`}>
                                    {metric.efficiency}%
                                </td>
                            </tr>
...
// --------------------------------------------------------------------------------
// --- Operating Time Summary Table (NEW) ---
// --------------------------------------------------------------------------------

/**
/**
 * Hook for calculating Utilization (Rated Operating Time vs. Actual Operating Time)
 */
function useOperatingTimeMetrics(rawReportData, actualOperatingTimes) {
    ...
    return useMemo(() => {
        ...
        return phasesConfig.map(config => {
            ...
            // NOTE: We assume 'operatingTimeMins' is now returned by the API
            const ratedTime = Math.round(apiData.operatingTimeMins || 0); 
            const actualTime = Math.round(actualOperatingTimes[config.id] || 0);

            // Calculation: Utilization = (Actual Time / Rated Time) * 100
    ...
    }, [rawReportData, actualOperatingTimes]);
}


function OperatingTimeSummaryTable({ rawReportData, actualOperatingTimes, setActualOperatingTimes }) {
    const metrics = useOperatingTimeMetrics(rawReportData, actualOperatingTimes);

    const handleActualTimeChange = useCallback((id, value) => {
        const num = Math.round(Number(value || 0));
        setActualOperatingTimes(prev => ({ ...prev, [id]: num }));
    }, [setActualOperatingTimes]);

    const hasData = metrics.some(m => m.ratedTime > 0);
    
    // 🛑 If there is NO rated data, we return null, otherwise we render the table
    if (!hasData) return null;

    return (
        <div className="mt-8">
            <h2 className="text-xl font-bold text-purple-600 mb-4 flex items-center">
                Operating Time and Utilization (Minutes)
            </h2>
            <div className="overflow-x-auto shadow-md rounded-lg border border-gray-100">
                <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tr key={metric.phase} className="hover:bg-purple-50/50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-lg text-right align-middle">
                                    <input
                                        type="number"
                                        min="0"
                                        value={actualOperatingTimes[metric.id] ?? ''}
                                        onChange={(e) => handleActualTimeChange(metric.id, e.target.value)}
                                        className="w-32 text-lg text-gray-700 font-semibold text-right border border-gray-300 rounded-md p-1 focus:border-indigo-500 focus:ring-indigo-500 appearance-none no-spinners"
                                        aria-label={`Actual Operating Time for ${metric.phase}`}
                                    />
                                </td>
                                <td className={`px-6 py-4 whitespace-nowrap text-2xl font-extrabold text-right align-middle ${
                                    parseFloat(metric.utilization) >= 95 ? 'text-green-600' : 
                                    parseFloat(metric.utilization) >= 80 ? 'text-yellow-600' : 'text-red-600'
                                }`}>
                                    {metric.utilization}%
                                </td>
                            </tr>
...
export default function EfficiencyDashboard() {
  ...
  const [actualOperatingTimes, setActualOperatingTimes] = useState({}); 
  
  const [isLoading, setIsLoading] = useState(false);

  // Utility to initialize all inputs states
  const initializeStates = useCallback((data) => {
    ...
    // 🛑 CRITICAL FIX: Ensure initialization uses the correct API field: actualOperatingTimeMins
    setActualOperatingTimes({
        sewing: Math.round(data.sewing?.actualOperatingTimeMins || 0), 
        inspection100: Math.round(data.inspection100?.actualOperatingTimeMins || 0),
    });
  }, []);
  ...
  const handleApplyFilter = async (initialStart = startDate, initialEnd = endDate) => {
    ...
    try {
        ...
        // This fetch is now returning the operatingTimeMins and actualOperatingTimeMins
  ...
  // A simpler check: if we have any report data, try to render the tables.
            ...
            {isLoading ? loadingPlaceholder : hasDataForRange ? (
                <>
                    ...
                    <OperatingTimeSummaryTable 
                        rawReportData={efficiencyReportData}
                        actualOperatingTimes={actualOperatingTimes}
                        setActualOperatingTimes={setActualOperatingTimes}
                    />
                </>
            ) : (
